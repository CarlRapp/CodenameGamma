		  ...
		  uniform bool gReflectionEnabled) : SV_Target
{
	...
	if (normalMapSample.x != 0 || normalMapSample.y != 0 || normalMapSample.z != 0)
	...
	// Lighting.
	//
	uint totalLights = 0, dummy;
    gDirLightBuffer.GetDimensions(totalLights, dummy);
	...
	{   
		...
		DirectionalLight dligt;
		
		dligt.Color = float4(1.0f, 1.0f, 1.0f , 0);
		dligt.Direction = float3(-1.0f, -1.0f, 1.0f );
		...
		{
			float4 A, D, S;
			ComputeDirectionalLight(mat, gDirLights[0], bumpedNormalW, toEye, 
				A, D, S);
